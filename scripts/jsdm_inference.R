...
num.occ <- apply(ifelse(data$y<0,0,data$y),2,sum)
ord.num.occ <- order(num.occ,decreasing=TRUE)
n.taxa.lat <- sum(num.occ > frac.occ.lat*nrow(data$y))
...
if ( n.latent > 0 )
{
  data$sigma_beta_lat <- z.range.max    # PR: enlarged width of prior
  data$n_taxa_lat     <- n.taxa.lat
  data$ord_num_occ    <- ord.num.occ
  if ( n.latent > 1 )
  {
    data$n_latent                 <- n.latent
    data$ind_beta_lat_constraints <- ord.num.occ[10+1:n.latent]
  }
}
...
for ( i in 1:n.chain )
{
  ...
  if ( n.latent > 1 )
  {
    init[[i]][["beta_lat"]] <- matrix(1,nrow=n.latent,ncol=n.taxa.lat)
    for ( j in 1:(n.latent-1) )
    {
      for ( jj in 1:j ) init[[i]][["beta_lat"]][j+1,data$ind_beta_lat_constraints[jj]] <- 0
    }
  }
...
if ( n.latent > 0 )
{
  model.code <- paste(model.code,
                      "  real                             sigma_beta_lat;\n",
                      "  int                              n_taxa_lat;\n",
                      "  int                              ord_num_occ[n_taxa];\n",
                      sep="")
  if ( n.latent > 1 )
  {
    model.code <- paste(model.code,
                        "  int                            n_latent;\n",
                        "  int                            ind_beta_lat_constraints[n_latent];\n",
                        sep="")
  }
}                      
model.code <- paste(model.code,
...
if ( n.latent > 1 )
{
  ...
  model.code <- paste(model.code,
                      "  matrix[n_latent,n_taxa_lat]      beta_lat;\n",
                      sep="")
}
model.code <- paste(model.code,
                    "}\n\n", 
...
if ( n.latent > 1 )
{
  ...
  model.code <- paste(model.code,
                      ...
                      "    }\n",
                      "    for ( j in 1:n_taxa_lat ) {\n",
                      "      if ( ord_num_occ[j]==ind_beta_lat_constraints[k] )\n",
                      "        beta_lat[k,j] ~ normal(0,sigma_beta_lat) T[0,];\n",  # positive coefficient
                      "      else if ( k==2 && ord_num_occ[j]==ind_beta_lat_constraints[1] )\n",
                      "        beta_lat[k,j] ~ normal(0,0.01);\n",
                      "      else if ( k==3 && (ord_num_occ[j]==ind_beta_lat_constraints[1] || ord_num_occ[j]==ind_beta_lat_constraints[2]) )\n",
                      "        beta_lat[k,j] ~ normal(0,0.01);\n",
                      "      else if ( k==4 && (ord_num_occ[j]==ind_beta_lat_constraints[1] || ord_num_occ[j]==ind_beta_lat_constraints[2] || ord_num_occ[j]==ind_beta_lat_constraints[3]) )\n",
                      "        beta_lat[k,j] ~ normal(0,0.01);\n",
                      "      else\n",
                      ...
                      "  }\n",
                      "  if ( n_latent > 4 ) {\n",
                      "    print(\"*** unification of latent variables not yet implemented for n_latent > 4\");\n",
                      "  }\n",
                      sep="")
}
model.code <- paste(model.code,"\n",sep="")
...
if ( n.latent == 1 )
{
  ...
  {
    model.code <- paste(model.code,
                        "      z[i,ord_num_occ[j]] = z[i,ord_num_occ[j]] + x_lat[siteIND[i]]*beta_lat[j];\n",        
                        sep="")
  }
  else
  {
    model.code <- paste(model.code,
                        "      z[i,ord_num_occ[j]] = z[i,ord_num_occ[j]] + x_lat[i]*beta_lat[j];\n",        
                        sep="")
  }
  model.code <- paste(model.code,
                      "    }\n",
...
if ( n.latent > 1 )
{
  ...
  if ( lat.site )
  {
    model.code <- paste(model.code,
                        "      for ( k in 1:n_latent ) {\n",
                        "        z[i,ord_num_occ[j]] = z[i,ord_num_occ[j]] + x_lat[siteIND[i],k]*beta_lat[k,j];\n",
                        "      }\n",
                        sep="")
  }
  else
  {
    model.code <- paste(model.code,
                        "      for ( k in 1:n_latent ) {\n",
                        "        z[i,ord_num_occ[j]] = z[i,ord_num_occ[j]] + x_lat[i,k]*beta_lat[k,j];\n",
                        "      }\n",
                        sep="")
  }
  model.code <- paste(model.code,
...
# save workspace:
save.image(file=paste("Inv_JSDM_",submodel,"_test2.RData",sep=""))
