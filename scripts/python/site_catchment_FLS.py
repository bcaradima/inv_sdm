...
results = root + "\\" + "resultsFLS.gdb" # output results
...
# Process the sites that are not already processed
for ID in inc:
...
##        env.snapRaster = fd
        ...
        # Get flow distance to stream
##        om = root + "\\" + "out_minus"
        inputRaster = SetNull(rivers, fd)
##        inputRaster.save(om)

        FL = arcpy.gp.FlowLength_sa(inputRaster, "DOWNSTREAM", "")
        FL = arcpy.Raster(FL)
        write('Flow -> | ')
