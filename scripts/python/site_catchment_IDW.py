...
results =  root + "\\" + "resultsFLO.gdb" # raster fGDB
...
for p in power:
    for ID in siteID:
        ...
        FN = fileName[i]
        d.setdefault(ID, [0, 0])
        rdist = results + "\\" + "fd_" + str(FN)
        tempFC = "catchment_dissolve"
        ...
        if arcpy.Exists(rdist) == True:
            ...
            if site_catchment[ID] == None:
                ...
                try:
                    raster_ed = arcpy.Raster(rdist)

                    # Dissolve sub-catchments to yield site catchment
                    arcpy.Dissolve_management(catchments, tempFC, "", "", "MULTI_PART", "")
                    ...
                    con_lu = Con(raster_lu, raster_ed, raster_lu, "Value = 1")

                    targetDist = arcpy.RasterToNumPyArray(con_lu, nodata_to_value = -1)
                    ...
                    totalDist = arcpy.RasterToNumPyArray(raster_ed, nodata_to_value = -1)
...
solutions = root + "\\" + "site_IDW_FLO_" + str(abs(p)) + "_" + land_use + ".csv"
