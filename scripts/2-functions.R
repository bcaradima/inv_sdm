...
# Given a vector of objects, remove all other objects in the environment
keep <- function(objects){
  env.objects <- ls()
  ind.objects <- match(objects, env.objects)
  del.objects <- env.objects[-ind.objects]
  rm(list = del.objects)
}
...
labeller.beta <- function(inf.fact){
...
taxon.rank <- function(taxon, print = TRUE){
  ...
  # Construct a diagonal matrix of logical elements (TRUE/FALSE) representing
  # taxonomic hierarchy
  taxonomic.ranks <- c("Phylum", "Class", "Order", "Family", "Genus", "Species")
  hierarchy <- matrix(nrow=length(taxonomic.ranks), ncol=length(taxonomic.ranks))
  hierarchy <- lower.tri(hierarchy, diag = TRUE)
  row.names(hierarchy) <- taxonomic.ranks
  colnames(hierarchy) <- taxonomic.ranks
  rm(taxonomic.ranks)
  
  taxon.rank.logical <- apply(inputs$taxonomy, 2, function(rank){
    taxon %in% rank
  })
  # What is the highest taxonomic level of the identified taxon?
  rank <- names(taxon.rank.logical[taxon.rank.logical]) 
...
run.isdm <- function(cm, predictors, trace=FALSE, messages=TRUE) {
  ...
  probability <- lapply(taxa, function(j){
    ...
    dt <- as_tibble(mdata[, c("SiteId", "SampId", j)])
...
cv.isdm <- function(predictors){
  ...
  for (fold in 1:folds) {
    ...
    train.data <- as_tibble(get(paste("train",fold,sep="")))
    test.data <- as_tibble(get(paste("test",fold,sep="")))
...
cv.jsdm <- function(directory = "outputs/paper 1 submission 2 results", folder) {
  require(mvtnorm)
  ...
  for (fold in 1:3){
    ...
    if (extensions$site.effects & extensions$n.latent){
      ...
      # Calculate linear predictor for multiple latent variables
        ...
        # Latent variables are summed by dimensions 1 and 3 (i.e., by "sites" or samples and taxa)
        z <- matrix(rep(alpha.taxa.maxpost,nrow(x)),nrow=nrow(x),byrow=TRUE) +
...
# Cross-validation of mSDMs and jSDMs based on the posterior sample
...
# - relies on proper naming of workspace filename to indicate model extensions (see identify.jsdm() function)
# - can calculate probabilities of occurences, deviance, and richness of the community and taxa under cross-validation
cv.jsdm.sample <- function(directory = "outputs/paper 1 submission 2 results", folder) {
  require(mvtnorm)
  ...
  for (fold in 1:3){
    ...
    for (k in 1:sampsize.eval){
      ...
      posterior.sample <- ind.sel.eval[k]
      train.z[k,,] <- matrix(rep(alpha[posterior.sample,],train.n.samples),byrow=TRUE,nrow=train.n.samples) +
        train.x %*% beta[posterior.sample,,]
      
      test.z[k,,] <- matrix(rep(alpha[posterior.sample,],test.n.samples),byrow=TRUE,nrow=test.n.samples) +
        test.x %*% beta[posterior.sample,,]
      
      # Add the site effect
      if (extensions$site.effects==1){
        g <- gamma[posterior.sample, train.siteIND]
        train.z[k,,] <- train.z[k,,] + matrix(rep(g,train.n.taxa),byrow=FALSE,nrow=train.n.samples)
        test.z[k,,] <- test.z[k,,] + matrix(rep(rnorm(test.n.samples,0,sigma.gamma[posterior.sample]),test.n.taxa),byrow=FALSE,nrow=test.n.samples)
      }
      
      # Check for any latent variables
      if (extensions$n.latent > 0){
        if (extensions$n.latent == 1){
          cov.resid[k,,]  <- beta.lat[posterior.sample,] %*% t(beta.lat[posterior.sample,])
        } else{
          cov.resid[k,,]  <- t(beta.lat[posterior.sample,,]) %*% beta.lat[posterior.sample,,]
        }
        # site-specific latent variables
        if (extensions$lat.site==1){ # test if latent variable is site- or sample-specific
          stop("Site-specific latent variables not implemented.")
          if (extensions$n.latent == 1){
            # train.z[k,,] <- train.z[k,,] + x.lat[posterior.sample, train.siteIND] %*% t(beta.lat[posterior.sample,])
          } else{
            # train.z[k,,] <- train.z[k,,] + x.lat[posterior.sample, train.siteIND,] %*% beta.lat[posterior.sample,,]
          }
          # sample-specific latent variables
        } else{
          if (extensions$n.latent == 1){
            # train.z[k,,] <- train.z[k,,] + sapply(beta.lat[posterior.sample,], function(i){ i * x.lat[posterior.sample,] })
            train.z[k,,] <- train.z[k,,] + x.lat[posterior.sample,] %*% t(beta.lat[posterior.sample,]) 
            # train.z[k,,] <- train.z[k,,] + x.lat[posterior.sample,] %*% beta.lat[posterior.sample,]
            # dim(train.z[k,,])
            # length(x.lat[posterior.sample,])
            # length(beta.lat[posterior.sample,])
          } else{
            # Modified chains and fixed posterior sample for TT2 returned error:
            # Error in train.z[k, , ] + sapply(beta.lat[posterior.sample, , ], function(i) { : non-conformable arrays
            # xb.lat <- sapply(beta.lat[posterior.sample,,], function(i){ i * x.lat[posterior.sample,,] })
            # train.z[k,,] <- train.z[k,,] + xb.lat
            
            # dim(train.z)
            # dim(beta.lat)
            # dim(beta.lat[posterior.sample,,])
            # dim(x.lat)
            # dim(x.lat[posterior.sample,,])
...
extract.vsr <- function(dir, sample){
  image <- new.env()
  
  load(paste(dir, '/variable_selection.RData',sep=''), envir = image)
  output <- image$output
  n <- occur.freq(sample)
  rm(image)
...
prop.jsdm.pred <- function(jsdm, get.quantiles=TRUE, quantiles=c(0.05, 0.95)){
  # Extract sites, samples, observations, inputs, and names of the inputs
  sites <- jsdm$sites
  ...
  m <- array(
    ...
    # e.g., dim=c(10000, 580, 245) for BDM community
  )
  ...
  # Warning message as of Feb 2020:
  #   In melt(prob) :
  #   The melt generic in data.table has been passed a array and will attempt to redirect to the relevant reshape2 method; please note that reshape2 is deprecated, and this redirection is now deprecated as well. To continue using melt methods from reshape2 while both libraries are attached, e.g. melt.list, you can prepend the namespace like reshape2::melt(prob). In the next version, this warning will become an error.
  # note: replace with tidyr::gather() or tidyr::pivot_longer()
  prob <- melt(prob)
...
# Examples of input arguments to test the function
# directory = "outputs/paper 1 submission 2 results"
# folder <- "FF0"
effective.parameters <- function(directory = "outputs/paper 1 submission 2 results", folder) {
  require(mvtnorm)
  # output = list("deviance" = tibble(), "probability" = tibble(), "richness" = tibble())
  output = list("deviance" = tibble())
  ...
  # Extract observations and inputs for calibration
  x <- as.matrix(model.image$env.cond[,inf.fact])
  colnames(x) <- inf.fact
  y <- as.matrix(model.image$occur.taxa)
  ...
  # Number of sites, samples, and occurrences and samples per taxon
  n.sites <- length(unique(sites))
  n.samples <- length(unique(samples))
  n.taxa <- ncol(y)
  
  n.present <- apply(y, 2, sum, na.rm = TRUE)
  n.sample.taxa <- apply(y, 2, function(j){sum(!is.na(j))})
  
  # Obtain posterior sample indices and sample size
  thin.eval <- 5
  ind.sel.eval <- thin.eval*(1:floor(dim(alpha)[1]/thin.eval))
  ind.sel.eval <- ind.sel.eval[1:500]
  sampsize.eval <- length(ind.sel.eval)
  ...
  # Calculate null probabilities and deviances 
  # Same for all posterior samples? presumably yes, since no posterior parameter distributions are involved)
  p.null <- apply(y, 2, function(j){sum(j, na.rm=TRUE)/sum(!is.na(j))})
  p.null <- ifelse(p.null==0,1e-4,p.null)
  p.null <- matrix(rep(p.null,nrow(y)),nrow=nrow(y),byrow=TRUE)
  
  # Pre-allocate data structures for results
  z <- array(dim=c(sampsize.eval,n.samples,n.taxa),dimnames=list(ind.sel.eval,1:dim(data$y)[1],dimnames(data$y)[[2]]))
  p <- z
  
  # Pre-allocation for deviance data structures
  deviance.null <- matrix(data = -2*sum(y*log(p.null)+(1-y)*log(1-p.null), na.rm = T),
                          nrow=nrow(y), ncol=ncol(y))
  deviance.null.taxa <- -2*apply(y*log(p.null)+(1-y)*log(1-p.null),2,sum,na.rm=T)
  
  deviance.resid <- array(dim=c(length(ind.sel.eval), nrow(y), ncol(y)), dimnames = list(ind.sel.eval, samples, colnames(y)))
  
  deviance.resid.taxa <- matrix(data = NA, nrow=length(ind.sel.eval), ncol=ncol(y))
  colnames(deviance.resid.taxa) <- colnames(y)
  
  # mean posterior ####
  # Calculate the deviance based on the mean posterior parameter values
  alpha.mean <- apply(alpha, 2, mean)
  beta.mean <- apply(beta, c(2,3), mean)
  
  z.mean <- matrix(rep(alpha.mean,n.samples),byrow=T,nrow=n.samples) + x %*% beta.mean
  
  # Add the site effect
  if ( extensions$site.effects==1 ){
    gamma.mean <- apply(gamma[, siteIND], 2, mean)
    z.mean <- z.mean + gamma.mean
  }
  ...
  if ( extensions$n.latent>0 ){
    if ( extensions$n.latent==1 ){
      ...
      x.lat.mean <- apply(x.lat, c(2,3), mean) # posterior samples are the first dimension
      beta.lat.mean <- apply(beta.lat, c(2,3), mean)
    }
    # site-specific latent variables: NOT IMPLEMENTED!
    if ( extensions$lat.site==1 ){ # test if latent variable is site- or sample-specific
      if ( extensions$n.latent == 1 ){
        stop("Site-specific latent variables not implemented.")
      } else{
      ...
      if ( extensions$n.latent == 1 ){
        z.mean <- z.mean + sapply(beta.lat.mean, function(i){ i*x.lat.mean})
      } else{
        # Multiple sample-specific latent variables
        z.mean <- z.mean + x.lat.mean %*% beta.lat.mean
      }
    }
  }
  
  p.mean <- 1/(1+exp(-z.mean))
  
  output$deviance.meanpar <- calc.deviance(y, p.mean)
  
  # posterior sample ####
  # Begin sampling
  cat("\nsampling                                                   v\n")
  for (k in 1:sampsize.eval){
    if (k %% floor(sampsize.eval/50) == 0) cat("*") # progress bar
    
    posterior.sample <- ind.sel.eval[k]
    
    # calculate z = \alpha + x*\beta
    z[k,,] <- matrix(rep(alpha[posterior.sample,],n.samples),byrow=TRUE,nrow=n.samples) +
      x %*% beta[posterior.sample,,]
    
    # Add the site effect
    if (extensions$site.effects==1){
      g <- gamma[posterior.sample, siteIND]
      z[k,,] <- z[k,,] + matrix(rep(g,n.taxa),byrow=FALSE,nrow=n.samples)
    }
    ...
    if (extensions$n.latent > 0){
      if (extensions$lat.site==1){
        stop("Site-specific latent variables not implemented.")
        if (extensions$n.latent == 1){
          # Single site-specific latent variable (expanded to samples)
          # z[k,,] <- z[k,,] + x.lat[posterior.sample, siteIND] %*% t(beta.lat[posterior.sample,])
        }
        if (extensions$n.latent > 1){
          # Multiple site-specific latent variables (expanded to samples)
          # z[k,,] <- z[k,,] + x.lat[posterior.sample, siteIND,] %*% beta.lat[posterior.sample,,]
        }
      }
      if (extensions$lat.site==0){
        # Single sample-specific latent variable
        if (extensions$n.latent == 1){
          z[k,,] <- z[k,,] + x.lat[posterior.sample,] %*% t(beta.lat[posterior.sample,])
        }
        # Multiple sample-specific latent variables
        if (extensions$n.latent > 1){
          z[k,,] <- z[k,,] + x.lat[posterior.sample,,] %*% beta.lat[posterior.sample,,]
        }
      }
    }
    ...
    # results: deviance
    # Posterior sample deviance of the proposed model
    # deviance.resid[k,,] <- calc.deviance(y, p[k,,])
    deviance.resid[k,,] <- sign(y-0.5)*sqrt(-2*(y*log(p[k,,])+(1-y)*log(1-p[k,,])))
    # 
    # # Sum of the squared residual deviances for all taxa
    deviance.resid.taxa[k, ] <- apply(deviance.resid[k,,]^2, 2, sum, na.rm=TRUE)
    
    # # Calculate standardized deviance and D^2
    # std.deviance[k,] <- deviance.resid[k,]/n.sample.taxa
    # d[k,] <- 1-deviance.resid[k,]/deviance.null.taxa
  }
  cat("\n\n")
  
  # combine deviance results
  # *.deviance.null.taxa : null deviance per taxon
  # *.deviance.resid.taxa : residual deviance per taxon
  # *.std.deviance: standardized residual deviance per taxon
  deviance <- melt(deviance.resid.taxa)
  deviance <- as_tibble(deviance)
  colnames(deviance) <- c("posterior.sample", "Taxon", "residual.deviance")
  
  deviance$ind.sel.eval <- ind.sel.eval[deviance$posterior.sample]
  deviance$null.deviance <- deviance.null.taxa[deviance$Taxon]
  deviance$n.present <- n.present[deviance$Taxon]
  deviance$n.samples <- n.sample.taxa[deviance$Taxon]
  deviance$Trial <- folder
...
# Given a result from effective.parameters() function, calculate the mean deviance over the posterior sample, and the deviance at the mean posterior parameter values
# Returns a list of the desired statistics according to Gelman et al (see paper 1 for details on these statistics)
calc.pd <- function(result){
  # Sum the deviance of all taxa at each posterior sample, then take the mean
  sum.deviance <- result$deviance %>%
    filter(!(is.infinite(residual.deviance))) %>%
    group_by(posterior.sample) %>% 
    summarize(sum.deviance = sum(residual.deviance))
  mean.deviance <- mean(sum.deviance$sum.deviance)
  
  # Sum the deviance based on the mean posterior parameter values
  mean.posterior.par.deviance <- result$deviance.meanpar
  
  output <- list("mean.deviance" = mean.deviance, 
                 "mean.posterior.par.deviance" = mean.posterior.par.deviance,
                 "pD.1" = mean.deviance - mean.posterior.par.deviance, 
                 "pD.2" = 0.5*var(sum.deviance$sum.deviance))
  return(output)
}
...
linear.predictor <- function(jsdm){
  ...
  if(jsdm$extensions$n.latent==1){
    ...
    dt <- as_tibble(jsdm$xb.lat.samples)
...
# Outputs a ggplot object
map.jsdm.pred.taxon <- function(jsdm, taxon, italic.title = TRUE, d.squared = TRUE, legend=TRUE){
  ...
  if(d.squared){
    ...
    if(italic.title){
      label <- bquote(italic(.(taxon.label))~"(D"^2 == .(signif(d.squared[taxon], 2))*")")
    } else{
      label <- bquote(.(taxon.label)~"(D"^2 == .(signif(d.squared[taxon], 2))*")")
    }
  } else{
    if(italic.title){
      label <- bquote(italic(.(taxon.label)))
    } else{
      label <- taxon.label
    }
  }
...
# # Plot map of influence factors
# # Requires: predictors, inputs$ch, inputs$xy, inv
map.inputs <- function(K, sample.data, plot.title="datasetName!", fileName) {
  x <- prepare.inputs(K, y = sample.data, center = FALSE)
  
  x <- gather(x, Variable, Value, -SiteId, -SampId)
  # Get predictor names without "SiteId"
  variables <- unique(x$Variable)
  variables <- variables[!(variables %in% c("Temp2", "FV2"))]
  ...
  for (k in 1:length(variables)){
    ...
    plot.data <- filter(x, Variable == variable)
    ...
    g <- g + geom_point(data = plot.data, aes(X, Y, size = Value), alpha = 0.35, color="black")
...
plot.prob <- function(jsdm, filename){
  ...
  pdf(paste(filename, '.pdf', sep=''), paper = 'special', height = 9, width = 12, onefile = TRUE, compress = TRUE)
  for (j in 1:length(taxa)){
    taxon <- taxa[j]
